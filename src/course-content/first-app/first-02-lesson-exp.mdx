...
Objects are a way of holding pairs of data that are all related.

For instance, here are two objects of possible users of your app.
...
For these users, we have seen the three main types of data: numbers, strings (text wrapped in strings), and booleans (the values true and false).

The value for a property can also be an object, as well. So we could have an object full of users that has embedded objects.
```js
let appUsers = {
  userOne: {
    firstName: "Liam",
...
Take a moment to look at the syntax differences between having userOne and userTwo as separate objects versus being object embedded in a larger object. 
In the second example we see a : replacing the = and a , replacing the ;.

Now when we look at the StyleSheet, we can see that it is an object with embedded objects for each separate style. 
